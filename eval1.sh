...
# Check if run1.sh exists
if [ ! -f "run1.sh" ]; then
    echo "❌ Error: run1.sh not found!"
    echo "Please create a run1.sh script that takes three parameters:"
    echo "  ./run1.sh <trip_duration_days> <miles_traveled> <total_receipts_amount>"
    echo "  and outputs the reimbursement amount"
    exit 1
fi

# Make run1.sh executable
chmod +x run1.sh
...
for ((i=0; i<num_cases; i++)); do
    ...
    if script_output=$(./run1.sh "$trip_duration" "$miles_traveled" "$receipts_amount" 2>/dev/null); then
        ...
        error_msg=$(./run1.sh "$trip_duration" "$miles_traveled" "$receipts_amount" 2>&1 >/dev/null | tr -d '\n')
...
echo "  2. Ensure your run1.sh outputs only a number"
